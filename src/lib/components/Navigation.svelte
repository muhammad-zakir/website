...
>
	...
	{#if isMobileMenuOpen}
		<button
			class="fixed inset-0 z-30 bg-graphite-950/60 backdrop-blur-sm md:hidden"
	...
	<!-- Mobile Drawer -->
	<div
		class="fixed top-0 right-0 z-40 flex h-full w-72 flex-col border-l border-graphite-700/50 bg-graphite-900/95 backdrop-blur-md transition-transform duration-300 md:hidden {isMobileMenuOpen
			? 'translate-x-0'
			: 'translate-x-full'}"
	>
		<div class="flex flex-col gap-1 px-4 pt-20 pb-6">
			{#each NAVIGATION_LINKS as link}
				<a
					href={link.href}
